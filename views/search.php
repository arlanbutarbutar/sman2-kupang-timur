...
if ($_SESSION['page-url'] == "jadwal") {
  if (isset($_GET['key']) && $_GET['key'] != "") {
    ...
    foreach ($keys as $no => $data) {
      $data = strtolower($data);
      $quer .= "jadwal.kelas LIKE '%$data%' OR guru.mapel LIKE '%$data%' OR guru.nama LIKE '%$data%'";
      if ($no + 1 < count($keys)) {
        $quer .= " OR ";
      }
    }
    $query = "SELECT * FROM jadwal JOIN guru ON jadwal.id_guru=guru.id_guru WHERE $quer ORDER BY jadwal.id_jadwal DESC LIMIT 100";
...
if ($_SESSION['page-url'] == "siswa") {
  if (isset($_GET['key']) && $_GET['key'] != "") {
    ...
    foreach ($keys as $no => $data) {
      $data = strtolower($data);
      $quer .= "nis LIKE '%$data%' OR nama LIKE '%$data%'";
      if ($no + 1 < count($keys)) {
        $quer .= " OR ";
      }
    }
