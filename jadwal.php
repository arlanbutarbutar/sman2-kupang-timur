  ...
  <div id="learn" class="learn">
    <div class="container">
          ...
          <div class="titlepage">
            ...
            <?php if (mysqli_num_rows($jadwal_belajar) > 0) { ?>
              ...
              <div class="table-responsive mt-4">
                <table class="table table-striped table-hover table-borderless table-sm text-center w-25">
                  <thead>
                    <tr>
                  ...
                  <tbody>
                    <?php while ($row_belajar = mysqli_fetch_assoc($jadwal_belajar)) { ?>
                      <tr>
                        <td><?= $row_belajar['rombel'] ?></td>
                        <td><?= $row_belajar['tahun'] ?></td>
                      </tr>
                    <?php } ?>
                  </tbody>
                </table>
      ...
      <div class="row">
        <div class="col-md-12">
          <div class="learn_box">
            <?php if (mysqli_num_rows($jadwal_pdf) > 0) {
              while ($row_pdf = mysqli_fetch_assoc($jadwal_pdf)) {
                $namaFile = $row_pdf['file'];
                $ekstensiGambarValid = ['jpg', 'png', 'jpeg', 'heic'];
                $ekstensiGambar = explode('.', $namaFile);
                $ekstensiGambar = strtolower(end($ekstensiGambar));
                if (!in_array($ekstensiGambar, $ekstensiGambarValid)) { ?>
                  <embed type="application/pdf" src="assets/file/jadwal/<?= $row_pdf['file'] ?>" style="width: 100%;height: 450px;"></embed>
                <?php } else { ?>
                  <img src="assets/file/jadwal/<?= $row_pdf['file'] ?>" class="img-fluid rounded-0" style="width: 100%;height: auto;" alt="">
            <?php }
              }
            } ?>
          </div>
